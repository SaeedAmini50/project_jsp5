...
PreparedStatement ps = con.prepareStatement("SELECT sum(quantity_item),sum(price_item),sum(product_benefit) FROM orders INNER JOIN product INNER JOIN order_item ON  product.product_ID = order_item.product_ID AND orders.order_ID = order_item.order_ID where DATE(order_date) = CURDATE();");
